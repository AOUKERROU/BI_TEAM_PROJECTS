...
public class JOB_STOCK_HEBDO implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "JOB_STOCK_HEBDO.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(JOB_STOCK_HEBDO.class);
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					while (rs_tDBInput_2.next()) {
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "row3", "tDBInput_2", "PORTAIL_FOURNISSEUR_AZURE", "tMSSqlInput", "tHashOutput_1",
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row3", 2, 0,
						"tDBInput_2", "PORTAIL_FOURNISSEUR_AZURE", "tMSSqlInput", "tHashOutput_1", "Fournisseur",
						"tHashOutput", "output")) {
					talendJobLogProcess(globalMap);
				}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (conn_tDBClose_4 != null && !conn_tDBClose_4.isClosed()) {
					if (log.isDebugEnabled())
						log.debug("tDBClose_4 - " + ("Closing the connection ") + ("conn_tDBConnection_2")
								+ (" to the database."));
					conn_tDBClose_4.close();
					if (log.isDebugEnabled())
						log.debug("tDBClose_4 - " + ("Connection ") + ("conn_tDBConnection_2")
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String insert_tDBOutput_1 = "INSERT INTO " + tableName_tDBOutput_1
						+ " (CODE_ENSEIGNE,CODE_INTERNE_ARTICLE,CODE_ARTICLE,CODE_SITE,LIB_ARTICLE,EAN,VALO_STOCK,QTE_STOCK,DATE_STOCK,LAST_MAJ_DATE) VALUES (?,?,?,?,?,?,?,?,?,?)";
					...
					class BytesLimit65535_tAggregateRow_1_AGGOUT {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tAggregateRow_1_AGGOUT.append("GROUPBYS" + " = " + "[{OUTPUT_COLUMN="
									+ ("FOUCIN") + ", INPUT_COLUMN=" + ("FOUCIN") + "}, {OUTPUT_COLUMN=" + ("NOM_FRNS")
									+ ", INPUT_COLUMN=" + ("NOM_FRNS") + "}, {OUTPUT_COLUMN=" + ("CNUF")
									+ ", INPUT_COLUMN=" + ("CNUF") + "}]");
							log4jParamters_tAggregateRow_1_AGGOUT.append(" | ");
							log4jParamters_tAggregateRow_1_AGGOUT.append("OPERATIONS" + " = " + "[{OUTPUT_COLUMN="
									+ ("CountFRN") + ", INPUT_COLUMN=" + ("FOUCIN") + ", IGNORE_NULL=" + ("false")
									+ ", FUNCTION=" + ("count") + "}]");
							log4jParamters_tAggregateRow_1_AGGOUT.append(" | ");
...
// ###############################
					...
					class BytesLimit65535_tDBInput_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_1.append("QUERY" + " = "
									+ "\"select * FROM (  SELECT S.SOCCHAIN CODEENSEIGNE,         U.ARVCINR CODE_INTERNE_ARTICLE,         U.ARVCEXR CODE_ARTICLE,         S.SOCSITE CODESITE,         REPLACE(PKSTRUCOBJ.GET_DESC(ARVCINR, 'FR'), '\\\\\\\"\\\\\\\"', '') LIBARTICLE,         PKARTCOCA.GET_CLOSESTEAN(U.ARVCINV) EAN,         PKSTOCK.GETSTOCKENVALREVADATE(S.SOCSITE,                                       U.ARVCINV,                                       TO_CHAR(TRUNC(SYSDATE, 'IW'),                                               'dd-MM-yyyy')) VALO_STOCK,         PKSTOCK.GETSTOCKENQTEVALADATE(S.SOCSITE,                                       U.ARVCINV,                                       TO_CHAR(TRUNC(SYSDATE, 'IW'),                                               'dd-MM-yyyy')) QTE_STOCK,         TRUNC(SYSDATE, 'IW') DATE_STOCK,         TO_CHAR(SYSDATE, 'yyyy-MM-dd Hh24:mm:ss') DATELASTUPDATEWEBJOB,         F.FCLCFIN FOUCIN    FROM SITDGENE S, ARTUV U, FOUCATALOG F   WHERE F.FCLCINV = U.ARVCINV     AND TRUNC(SYSDATE) BETWEEN F.FCLDDEB AND F.FCLDFIN     AND EXISTS   (SELECT 1            FROM STOCOUCH           WHERE STOSITE = S.SOCSITE             AND STOCINR = U.ARVCINR             AND STOCINL = U.ARVCINV             AND ROWNUM = 1)    )   WHERE QTE_STOCK > 0 \"");
							log4jParamters_tDBInput_1.append(" | ");
							...
							log4jParamters_tDBInput_1.append("TRIM_COLUMN" + " = " + "[{TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("CODE_ENSEIGNE") + "}, {TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("CODE_INTERNE_ARTICLE") + "}, {TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("CODE_ARTICLE") + "}, {TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("CODE_SITE") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN="
									+ ("LIB_ARTICLE") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("EAN")
									+ "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("VALO_STOCK") + "}, {TRIM="
									+ ("false") + ", SCHEMA_COLUMN=" + ("QTE_STOCK") + "}, {TRIM=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("DATE_STOCK") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN="
									+ ("LAST_MAJ_DATE") + "}, {TRIM=" + ("false") + ", SCHEMA_COLUMN=" + ("FOUCIN")
									+ "}]");
							log4jParamters_tDBInput_1.append(" | ");
				...
				String dbquery_tDBInput_1 = "select * FROM (\nSELECT S.SOCCHAIN CODEENSEIGNE,\n       U.ARVCINR CODE_INTERNE_ARTICLE,\n       U.ARVCEXR CODE_ARTICLE"
						+ ",\n       S.SOCSITE CODESITE,\n       REPLACE(PKSTRUCOBJ.GET_DESC(ARVCINR, 'FR'), '\\\"\\\"', '') LIBARTICLE,\n       PK"
						+ "ARTCOCA.GET_CLOSESTEAN(U.ARVCINV) EAN,\n       PKSTOCK.GETSTOCKENVALREVADATE(S.SOCSITE,\n                               "
						+ "      U.ARVCINV,\n                                     TO_CHAR(TRUNC(SYSDATE, 'IW'),\n                                  "
						+ "           'dd-MM-yyyy')) VALO_STOCK,\n       PKSTOCK.GETSTOCKENQTEVALADATE(S.SOCSITE,\n                                "
						+ "     U.ARVCINV,\n                                     TO_CHAR(TRUNC(SYSDATE, 'IW'),\n                                   "
						+ "          'dd-MM-yyyy')) QTE_STOCK,\n       TRUNC(SYSDATE, 'IW') DATE_STOCK,\n       TO_CHAR(SYSDATE, 'yyyy-MM-dd Hh24:m"
						+ "m:ss') DATELASTUPDATEWEBJOB,\n       F.FCLCFIN FOUCIN\n  FROM SITDGENE S, ARTUV U, FOUCATALOG F\n WHERE F.FCLCINV = U.AR"
						+ "VCINV\n   AND TRUNC(SYSDATE) BETWEEN F.FCLDDEB AND F.FCLDFIN\n   AND EXISTS\n (SELECT 1\n          FROM STOCOUCH\n      "
						+ "   WHERE STOSITE = S.SOCSITE\n           AND STOCINR = U.ARVCINR\n           AND STOCINL = U.ARVCINV\n           AND ROW"
						+ "NUM = 1)\n  )\n WHERE QTE_STOCK > 0 ";
...
// Start of branch "out1"
							if (out1 != null) {
								...
								if (out1.CODE_ENSEIGNE == null) {
									...
									pstmt_tDBOutput_1.setLong(1, out1.CODE_ENSEIGNE);
								}
								...
								if (out1.CODE_ARTICLE == null) {
									...
									pstmt_tDBOutput_1.setLong(3, out1.CODE_ARTICLE);
								}

								if (out1.CODE_SITE == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setLong(4, out1.CODE_SITE);
								}

								if (out1.LIB_ARTICLE == null) {
									pstmt_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(5, out1.LIB_ARTICLE);
								}

								if (out1.EAN == null) {
									pstmt_tDBOutput_1.setNull(6, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setLong(6, out1.EAN);
								}
								...
								if (out1.DATE_STOCK != null) {
									pstmt_tDBOutput_1.setObject(9, new java.sql.Timestamp(out1.DATE_STOCK.getTime()),
											java.sql.Types.DATE);
								} else {
								...
								if (out1.LAST_MAJ_DATE != null) {
									pstmt_tDBOutput_1.setObject(10,
											new java.sql.Timestamp(out1.LAST_MAJ_DATE.getTime()), java.sql.Types.DATE);
								} else {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (log.isDebugEnabled()) {
					class BytesLimit65535_tDBOutput_3 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBOutput_3.append(" | ");
							log4jParamters_tDBOutput_3.append("FIELD_OPTIONS" + " = " + "[{UPDATE_KEY=" + ("false")
									+ ", DELETE_KEY=" + ("false") + ", UPDATABLE=" + ("true") + ", INSERTABLE="
									+ ("true") + ", SCHEMA_COLUMN=" + ("CODEENSEIGNE") + "}, {UPDATE_KEY=" + ("true")
									+ ", DELETE_KEY=" + ("false") + ", UPDATABLE=" + ("true") + ", INSERTABLE="
									+ ("true") + ", SCHEMA_COLUMN=" + ("CODE_INTERNE_ARTICLE") + "}, {UPDATE_KEY="
									+ ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE=" + ("true")
									+ ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("CODE_ARTICLE")
									+ "}, {UPDATE_KEY=" + ("true") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("CODESITE")
									+ "}, {UPDATE_KEY=" + ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("LIBARTICLE")
									+ "}, {UPDATE_KEY=" + ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("EAN")
									+ "}, {UPDATE_KEY=" + ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("VALO_STOCK")
									+ "}, {UPDATE_KEY=" + ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("QTE_STOCK")
									+ "}, {UPDATE_KEY=" + ("true") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
									+ ("true") + ", INSERTABLE=" + ("true") + ", SCHEMA_COLUMN=" + ("DATE_STOCK")
									+ "}, {UPDATE_KEY=" + ("false") + ", DELETE_KEY=" + ("false") + ", UPDATABLE="
				...
				try (java.sql.Statement stmtTrunc_tDBOutput_3 = conn_tDBOutput_3.createStatement()) {
					...
					if (log.isDebugEnabled())
						log.debug("tDBOutput_3 - " + ("Truncate") + (" table '") + ("[" + tableName_tDBOutput_3 + "]")
								+ ("' has succeeded."));
				...
				String insert_tDBOutput_3 = "INSERT INTO [" + tableName_tDBOutput_3
						+ "] ([CODEENSEIGNE],[CODE_INTERNE_ARTICLE],[CODE_ARTICLE],[CODESITE],[LIBARTICLE],[EAN],[VALO_STOCK],[QTE_STOCK],[DATE_STOCK],[DateLastUpdateWebJob]) VALUES (?,?,?,?,?,?,?,?,?,?)";
				pstmt_tDBOutput_3 = conn_tDBOutput_3.prepareStatement(insert_tDBOutput_3);
				resourceMap.put("pstmt_tDBOutput_3", pstmt_tDBOutput_3);

				boolean isShareIdentity_tDBOutput_3 = globalMap.get("shareIdentitySetting_tDBConnection_4") != null
						&& (Boolean) globalMap.get("shareIdentitySetting_tDBConnection_4") == true;
					...
					class BytesLimit65535_tUniqRow_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tUniqRow_1.append("UNIQUE_KEY" + " = " + "[{CASE_SENSITIVE=" + ("false")
									+ ", KEY_ATTRIBUTE=" + ("false") + ", SCHEMA_COLUMN=" + ("CODEENSEIGNE")
									+ "}, {CASE_SENSITIVE=" + ("false") + ", KEY_ATTRIBUTE=" + ("true")
									+ ", SCHEMA_COLUMN=" + ("CODE_INTERNE_ARTICLE") + "}, {CASE_SENSITIVE=" + ("false")
									+ ", KEY_ATTRIBUTE=" + ("false") + ", SCHEMA_COLUMN=" + ("CODE_ARTICLE")
									+ "}, {CASE_SENSITIVE=" + ("false") + ", KEY_ATTRIBUTE=" + ("true")
									+ ", SCHEMA_COLUMN=" + ("CODESITE") + "}, {CASE_SENSITIVE=" + ("false")
									+ ", KEY_ATTRIBUTE=" + ("false") + ", SCHEMA_COLUMN=" + ("LIBARTICLE")
									+ "}, {CASE_SENSITIVE=" + ("false") + ", KEY_ATTRIBUTE=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("EAN") + "}, {CASE_SENSITIVE=" + ("false")
									+ ", KEY_ATTRIBUTE=" + ("false") + ", SCHEMA_COLUMN=" + ("VALO_STOCK")
									+ "}, {CASE_SENSITIVE=" + ("false") + ", KEY_ATTRIBUTE=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("QTE_STOCK") + "}, {CASE_SENSITIVE=" + ("false")
									+ ", KEY_ATTRIBUTE=" + ("true") + ", SCHEMA_COLUMN=" + ("DATE_STOCK")
									+ "}, {CASE_SENSITIVE=" + ("false") + ", KEY_ATTRIBUTE=" + ("false")
									+ ", SCHEMA_COLUMN=" + ("DateLastUpdateWebJob") + "}]");
					...
					class BytesLimit65535_tDBInput_3 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBInput_3.append("QUERY" + " = "
									+ "\"SELECT DISTINCT   CODE_ENSEIGNE,    CODE_INTERNE_ARTICLE,    CODE_ARTICLE,    CODE_SITE,    LIB_ARTICLE,    EAN,    VALO_STOCK,    QTE_STOCK,    DATE_STOCK,    LAST_MAJ_DATE FROM  ODS_STOCK_HEBDO\"");
							log4jParamters_tDBInput_3.append(" | ");
...
// Start of branch "OUT_AZURE"
						if (OUT_AZURE != null) {
							...
							if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
								...
								row8.LIBARTICLE = OUT_AZURE.LIBARTICLE;
								row8.EAN = OUT_AZURE.EAN;
								row8.VALO_STOCK = OUT_AZURE.VALO_STOCK;
								row8.QTE_STOCK = OUT_AZURE.QTE_STOCK;
								row8.DATE_STOCK = OUT_AZURE.DATE_STOCK;
...
// Start of branch "row8"
							if (row8 != null) {
								...
								if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
										...
										, "row8", "tUniqRow_1", "tUniqRow_1", "tUniqRow", "tDBOutput_3",
								...
								if (row8.DATE_STOCK != null) {
									pstmt_tDBOutput_3.setTimestamp(9,
											new java.sql.Timestamp(row8.DATE_STOCK.getTime()));
								} else {
...
// ###############################      
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row8", 2, 0,
						"tUniqRow_1", "tUniqRow_1", "tUniqRow", "tDBOutput_3", "PORTAIL_FOURNISSEUR_AZURE",
						"tMSSqlOutput", "output")) {
					talendJobLogProcess(globalMap);
				}
					...
					class BytesLimit65535_tDBRow_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tDBRow_1.append("QUERY" + " = "
									+ "\"MERGE StockParArticle AS T  USING StockParArticleTT As S  ON S.CODESITE = T.CODESITE and S.CODE_INTERNE_ARTICLE = T.CODE_INTERNE_ARTICLE and S.DATE_STOCK = T.DATE_STOCK  WHEN NOT MATCHED THEN      INSERT (CODESITE,CODE_INTERNE_ARTICLE, DATE_STOCK,CODEENSEIGNE,CODE_ARTICLE,LIBARTICLE,EAN,VALO_STOCK,QTE_STOCK,DateLastUpdateWebJob)       VALUES (S.CODESITE,S.CODE_INTERNE_ARTICLE, S.DATE_STOCK,S.CODEENSEIGNE,S.CODE_ARTICLE,S.LIBARTICLE,S.EAN,S.VALO_STOCK,S.QTE_STOCK,SYSDATETIME())  WHEN MATCHED THEN UPDATE SET      T.VALO_STOCK = S.VALO_STOCK, T.CODEENSEIGNE = S.CODEENSEIGNE, T.LIBARTICLE= S.LIBARTICLE,      T.QTE_STOCK	= S.QTE_STOCK, T.CODE_ARTICLE= S.CODE_ARTICLE, T.EAN = S.EAN,   	DateLastUpdateWebJob= SYSDATETIME();  \"");
							log4jParamters_tDBRow_1.append(" | ");
	...
	public void tHashInput_4Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tFileOutputDelimited_2 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFileOutputDelimited_2.append("FILENAME" + " = "
									+ "context.PATH_STOCK_HEBDO+\"/FRNS_STOCK_HEBDO_\"+((String)globalMap.get(\"CurrentDate\"))+\".csv\"");
							log4jParamters_tFileOutputDelimited_2.append(" | ");
				...
				fileName_tFileOutputDelimited_2 = (new java.io.File(context.PATH_STOCK_HEBDO + "/FRNS_STOCK_HEBDO_"
...
// ###############################
				...
				org.talend.designer.components.hashfile.common.MapHashFile mf_tHashInput_4 = org.talend.designer.components.hashfile.common.MapHashFile
						.getMapHashFile();
				org.talend.designer.components.hashfile.memory.AdvancedMemoryHashFile<row3Struct> tHashFile_tHashInput_4 = mf_tHashInput_4
						.getAdvancedMemoryHashFile("tHashFile_JOB_STOCK_HEBDO_" + pid + "_tHashOutput_1");
				...
				while (iterator_tHashInput_4.hasNext()) {
					...
					if (tHash_Lookup_row10 != null && tHash_Lookup_row10.getCount(row10HashKey) > 1) { // G 071
						...
						// 'row10' and it contains more one result from keys : row10.FOUCFIN = '" +
						// row10HashKey.FOUCFIN + "'");
					} // G 071
...
// # Output table : 'copyOfOUTFRN'
						...
						log.debug("tMap_4 - Outputting the record " + count_copyOfOUTFRN_tMap_4
								+ " of the output table 'copyOfOUTFRN'.");
...
// Start of branch "copyOfOUTFRN"
					if (copyOfOUTFRN != null) {
						...
						if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
								...
								, "copyOfOUTFRN", "tMap_4", "tMap_4", "tMap", "tFileOutputDelimited_2",
						...
						if (copyOfOUTFRN.FOUCFIN_FOURNISSEUR != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.FOUCFIN_FOURNISSEUR);
						}
						sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
						if (copyOfOUTFRN.LIBFRS_FOURNISSEUR != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.LIBFRS_FOURNISSEUR);
						}
						sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
						if (copyOfOUTFRN.FOUCNUF_FOURNISSEUR != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.FOUCNUF_FOURNISSEUR);
						}
						sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
						if (copyOfOUTFRN.COUNTFRN != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.COUNTFRN);
						}
						sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
						if (copyOfOUTFRN.Date_Debut != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.Date_Debut);
						}
						sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
						if (copyOfOUTFRN.Date_Fin != null) {
							sb_tFileOutputDelimited_2.append(copyOfOUTFRN.Date_Fin);
						}
						...
						log.debug(
								"tFileOutputDelimited_2 - Writing the record " + nb_line_tFileOutputDelimited_2 + ".");
				...
				org.talend.designer.components.hashfile.common.MapHashFile.resourceLockMap
						.remove("tHashFile_JOB_STOCK_HEBDO_" + pid + "_tHashOutput_1");
...
// ###############################      
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "copyOfOUTFRN", 2, 0,
						"tMap_4", "tMap_4", "tMap", "tFileOutputDelimited_2", "tFileOutputDelimited_2",
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (log.isDebugEnabled()) {
					class BytesLimit65535_tFileExist_2 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFileExist_2.append("FILE_NAME" + " = "
									+ "((String)globalMap.get(\"tFileOutputDelimited_2_FILE_NAME\"))");
				...
				if (!file_tFileExist_2.exists()) {
					...
					log.info("tFileExist_2 - Directory or file : " + file_tFileExist_2.getAbsolutePath()
							+ " doesn't exist.");
				} else {
	...
	public void tSendMail_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tSendMail_2 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tSendMail_2.append("MESSAGE" + " = "
									+ "\"Hello,  Merci de trouver ci-joint les fournisseurs de l'alimentation du Stock Hebdo.  Cordialement\"");
							log4jParamters_tSendMail_2.append(" | ");
				...
				String cc_tSendMail_2 = (("aoukerrou@marjane.co.ma ; zjaddy@marjane.co.ma") == null
	...
	public void tHashInput_5Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tHashInput_5 - inputs:(after_tHashInput_4) outputs:(row10,row10)
				...
				org.talend.designer.components.hashfile.common.MapHashFile mf_tHashInput_5 = org.talend.designer.components.hashfile.common.MapHashFile
						.getMapHashFile();
				org.talend.designer.components.hashfile.memory.AdvancedMemoryHashFile<row4Struct> tHashFile_tHashInput_5 = mf_tHashInput_5
						.getAdvancedMemoryHashFile("tHashFile_JOB_STOCK_HEBDO_" + pid + "_tHashOutput_2");
				...
				while (iterator_tHashInput_5.hasNext()) {
					...
					if (runStat.update(execStat, enableLogStash, iterateId, 1, 1
							...
							, "row10", "tHashInput_5", "COUNTFRN", "tHashInput", "tAdvancedHash_row10",
				...
				org.talend.designer.components.hashfile.common.MapHashFile.resourceLockMap
						.remove("tHashFile_JOB_STOCK_HEBDO_" + pid + "_tHashOutput_2");
				...
				if (runStat.updateStatAndLog(execStat, enableLogStash, resourceMap, iterateId, "row10", 2, 0,
						"tHashInput_5", "COUNTFRN", "tHashInput", "tAdvancedHash_row10", "tAdvancedHash_row10",
						"tAdvancedHash", "output")) {
					talendJobLogProcess(globalMap);
				}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (conn_tDBClose_2 != null && !conn_tDBClose_2.isClosed()) {
					if (log.isDebugEnabled())
						log.debug("tDBClose_2 - " + ("Closing the connection ") + ("conn_tDBConnection_4")
								+ (" to the database."));
					conn_tDBClose_2.close();
					if (log.isDebugEnabled())
						log.debug("tDBClose_2 - " + ("Connection ") + ("conn_tDBConnection_4")
								+ (" to the database has closed."));
				}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = JOB_STOCK_HEBDO.class.getClassLoader()
					.getResourceAsStream("bi_team_projects/job_stock_hebdo_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = JOB_STOCK_HEBDO.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : JOB_STOCK_HEBDO");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 530659 characters generated by Talend Cloud Data Management Platform on the
 * 25 juillet 2022 à 14:20:42 WEST
 ************************************************************************************************/